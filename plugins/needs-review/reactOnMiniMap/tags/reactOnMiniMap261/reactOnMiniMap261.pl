...
sub onAction {
	...
	for ($k = 0; (exists $config{$configPrefix.$k}); $k++) {
		...
		@red = ();
		if (exists $config{$configPrefix.$k."_red"}) {
			if ($config{$configPrefix.$k."_red"} =~/\-+/) {
				@red = split /\s*\-+\s*/, $config{$configPrefix.$k."_red"};
			} else {
				$red[0] = $config{$configPrefix.$k."_red"};
			}
		} else {
			$red[0] = 0;
			$red[1] = 255;
		...
		if (exists $config{$configPrefix.$k."_green"}) {
			@green = ();
			if ($config{$configPrefix.$k."_green"} =~/\-+/) {
				@green = split /\s*\-+\s*/, $config{$configPrefix.$k."_green"};
			} else {
				$green[0] = $config{$configPrefix.$k."_green"};
			}
		} else {
			$green[0] = 0;
			$green[1] = 255;
		...
		if (exists $config{$configPrefix.$k."_blue"}) {
			@blue = ();
			if ($config{$configPrefix.$k."_blue"} =~/\-+/) {
				@blue = split /\s*\-+\s*/, $config{$configPrefix.$k."_blue"};
			} else {
				$blue[0] = $config{$configPrefix.$k."_blue"};
			}
		} else {
			$blue[0] = 0;
			$blue[1] = 255;
